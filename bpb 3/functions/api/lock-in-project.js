 ...
 *   2. Insert one row into public.signature_intents via Supabase REST using
 *      the service role key (RLS bypass). Returns 500 if the insert fails.
 *   3. Fetch proposal context (project_address, project_city, bid_total_amount).
 *   4. Best-effort: send a styled HTML+text email to Tim via Resend.
 *   5. Best-effort: create a JobNimbus contact tagged "Contract Pending".
 *   6. If either email or JN succeeded, PATCH notified_at on the row.
 *
 * Required env vars:
 *   SUPABASE_URL
 *   SUPABASE_SERVICE_ROLE_KEY
 *
 * Optional env vars (each independently enables a notification path):
 *   RESEND_API_KEY              — Resend API key
 *   RESEND_FROM_EMAIL           — verified sender, e.g.
 *                                 "Bayside Portal <tim@mcmullen.properties>"
 *   SIGN_INTENT_NOTIFY_EMAIL    — destination; default tim@mcmullen.properties
 *   JOBNIMBUS_API_KEY           — Bayside's JobNimbus admin API key
 */
...
export async function onRequestPost({ request, env }) {
  ...
  // ─── Fetch proposal context (used by both email + JN) ──────────────────
  let propRow = {};
  try {
  ...
  // ─── Best-effort: email Tim via Resend ─────────────────────────────────
  let emailOk = false;
...
<div style="font-family:-apple-system,BlinkMacSystemFont,'Inter',sans-serif;max-width:620px;margin:0 auto;padding:24px;color:#0e1218;background:#fff;">
  ...
  // ─── Best-effort: JobNimbus contact upsert ─────────────────────────────
  let jnOk = false;
  ...
  // ─── Mark notified_at if either path succeeded ─────────────────────────
