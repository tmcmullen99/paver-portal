...
if (!isPublicAdminPath()) {
  if (!hasLocalSession()) {
    ...
    supabase.auth.getSession().then(({ data, error }) => {
      if (error || !data || !data.session) { redirectToLogin(); return; }
      // SPRINT 7: workspace status enforcement. When a company's
      // subscription lapses (Stripe webhook sets companies.status =
      // 'suspended'), its STAFF are routed to a friendly paused page.
      // Homeowner portal pages don't load this module, so client access
      // to their proposals is intentionally unaffected.
      if (!window.location.pathname.startsWith('/suspended')) {
        supabase.rpc('get_my_workspace_status').then(({ data: st }) => {
          if (st === 'suspended') window.location.replace('/suspended.html');
        }).catch(() => {});
      }
